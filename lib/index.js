...
const modifyClassNames = function(classNames, modifiers) {
  ...
  // FIXME: implement properly somehow
  const names = classNames.trim().split(/\s+/).filter(className => !className.match(/__[^_]+_[^_]|[^_]_[^_]+__/));
  return (classNames + ' ' + names.map(className => modifiers.map(modifier => className + modifier).join(' ')).join(' ')).trim();
};

const createElemClassNames = function(classNames, elemNames) {
  // Quickfix to prevent excessive multiplying of nested bemto classNames
  // FIXME: implement properly somehow
  const names = classNames.trim().split(/\s+/).filter(className => !className.match(/__[^_]+_[^_]/));
  return (names.map(className => elemNames.map(elemName => className + '__' + elemName).join(' ')).join(' ')).trim();
};
...
const applyToAll = function(content, func, modifier) {
  ...
  if (modifier) {
    const modifyContent = function(content) {
      if (typeof content !== 'object' || !Object.isExtensible(content)) return content;
...
const compileProps = function(fromProps, toProps, tag, tagProps, acceptProps, component) {
  ...
  elemAccept && elemAccept.map(key => {
    if (tagProps[key] !== undefined && tagProps[key] !== null && !(isNotComponent && key[0] === '_') && key !== 'children' && key !== 'className') {
...
const bemto = function(tagString, additionalOptions) {
  ...
  const bemtoFactory = class Bemto extends React.Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
      if (blockOptions.lifecycle && blockOptions.lifecycle.getDerivedStateFromProps) {
        return blockOptions.lifecycle.getDerivedStateFromProps(nextProps, prevState);
      }
      return null;
    }
    componentDidMount() {
    ...
    shouldComponentUpdate(nextProps, nextState) {
      if (blockOptions.lifecycle && blockOptions.lifecycle.shouldComponentUpdate) {
        return blockOptions.lifecycle.shouldComponentUpdate(nextProps, nextState);
      }
      return true;
    }
    getSnapshotBeforeUpdate(prevProps, prevState) {
      if (blockOptions.lifecycle && blockOptions.lifecycle.getSnapshotBeforeUpdate) {
        return blockOptions.lifecycle.getSnapshotBeforeUpdate(prevProps, prevState, this);
      }
      return null;
    }
    componentDidUpdate(prevProps, prevState, snapshot) {
      if (blockOptions.lifecycle && blockOptions.lifecycle.componentDidUpdate) {
        blockOptions.lifecycle.componentDidUpdate(prevProps, prevState, snapshot, this);
      }
    }
