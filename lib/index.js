...
const renderModifier = function(key, value, blockProps) {
  ...
  if (type === 'function') {
    modValue = value(blockProps);
    if (modValue) {
      return renderModifier(key, modValue, blockProps);
    } else {
      return '';
    }
  }
...
const gatherModifiers = function(props, modifiersObj, blockProps) {
  ...
  const modifiers = propKeys.filter(prop => props[prop] && prop[0] === '_' && prop[1] !== '_').map(key => renderModifier(key, props[key], blockProps));
  if (modifiersObj) {
    for (var key in modifiersObj) {
      if (propKeys.indexOf(key) === -1) {
        const modValue = renderModifier(key, modifiersObj[key], blockProps);
        if (modValue) {
...
const selectTag = function(props, defaultTag, context, blockProps) {
  const tag = (typeof defaultTag === 'function' && !React.Component.isPrototypeOf(defaultTag)) ? defaultTag(blockProps) : defaultTag;
  if (blockProps && blockProps.__is) {
    if (typeof blockProps.__is === 'function' && !React.Component.isPrototypeOf(blockProps.__is)) {
      return blockProps.__is(blockProps);
    }
...
const collectOptions = function(tagString, additionalOptions) {
  ...
  options.acceptProps = options.acceptProps;
...
const compileProps = function(fromProps, toProps, tag, tagProps, acceptProps) {
  ...
  if (toProps) {
    for (var key in toProps) {
      if (toProps[key] && toProps[key].bemtoCalculated) {
        const calculatedValue = toProps[key].bemtoCalculated(tagProps);
...
const gatherOptions = function(outerOptions, baseOptions) {
  ...
  compileProps(fullProps, finalOptions.props, finalOptions.tag, baseOptions.tagProps, outerOptions.acceptProps)
  ...
  return {
    ...
    finalTag: selectTag(finalOptions.props, finalOptions.tag, finalOptions.tagContext, baseOptions.tagProps),
